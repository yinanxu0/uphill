...
class SubmitterThread(threading.Thread):
    def __init__(
        ...
        *iterables,
        ...
        super().__init__()
        if not callable(fn):
            loggerx.warning(f"{fn} is not callable, thread submitted failed.")
            return
        self.fn = fn
...
def parallel_for(
    ...
    *iterables: Iterable,
    ...
    """
    if not callable(fn):
        loggerx.warning(f"{fn} is not callable, parallel jobs failed.")
        return
...
def parallel_run(
    ...
    *iterables: Iterable,
    ...
    while thread.is_alive() or not q.empty():
        try:
            q.get(block=True, timeout=1).result()
        except queue.Empty:
            # Hit the timeout but thread is still running, try again.
            # This is needed to avoid hanging at the end when nothing else
            # shows up in the queue, but the thread didn't shutdown yet.
            continue
        # TODO: to modify the implementation
        # sleep 3 seconds to avoid thread competion, such as when make same parent 
        # directory using `uphill.io.file.unpack` unpack to same directory.
        # time.sleep(3)
    thread.join()
